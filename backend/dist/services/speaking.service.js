...
exports.getSpeakingService = getSpeakingService;
const cache_service_1 = require("./cache.service");
const openai_1 = __importDefault(require("openai"));
// ==================== IPA PHONEME MAPPINGS ====================
const PHONEME_MAP = new Map([
    // Vowels
    ['a', 'æ'], // cat
    ['e', 'ɛ'], // bed
    ['i', 'ɪ'], // sit
...
class SpeakingService {
     ...
     */
    async analyzePronunciation(userId, phraseId, audioBlob, expectedText) {
        try {
            ...
            // Cache result
            await this.cache.set(`speaking:${userId}:${phraseId}:latest`, analysis, { ttl: 3600 });
            return analysis;
        }
        catch (error) {
     ...
     */
    analyzePhonemes(expected, actual) {
        ...
        for (let i = 0; i < Math.min(expectedWords.length, actualWords.length); i++) {
            ...
            for (let j = 0; j < expPhonemes.length; j++) {
                const score = j < actPhonemes.length && expPhonemes[j] === actPhonemes[j]
                    ? 100
     ...
     */
    calculatePhonemeScore(expected, actual) {
        if (expected === actual)
            return 100;
     ...
     */
    detectMistakes(expected, actual) {
        const mistakes = [];
        const expectedWords = this.normalizeText(expected).split(' ');
        const actualWords = this.normalizeText(actual).split(' ');
        // Align words
     ...
     */
    calculateMistakeSeverity(expected, actual) {
     ...
     */
    async calculateFluency(_audio, transcription) {
     ...
     */
    calculateCompleteness(expected, actual) {
     ...
     */
    async analyzeProsody(_audio, _expectedText) {
    ...
    calculateOverallScore(scores) {
        ...
        return (scores.accuracy * weights.accuracy +
...
`;
            const response = await this.openai.chat.completions.create({
                ...
                messages: [
                    {
                        role: 'system',
                        content: 'You are a supportive English pronunciation coach who gives specific, actionable feedback.',
                    },
                    {
                        role: 'user',
                        content: prompt,
     ...
     */
    generateRecommendations(mistakes, phonemes) {
        ...
        if (weakPhonemes.length > 0) {
            recommendations.push(`Practice these sounds: ${weakPhonemes.slice(0, 3).join(', ')}`);
        }
        // High severity mistakes
        const criticalMistakes = mistakes.filter((m) => m.severity === 'high');
        if (criticalMistakes.length > 0) {
            recommendations.push(`Focus on: ${criticalMistakes[0].word} - ${criticalMistakes[0].suggestion}`);
        }
        // General tips
     ...
     */
    async trackProgress(userId, analysis) {

...
const teachersService = (0, teachers_service_1.getTeachersService)();
/**
...
router.get('/search', async (req, res) => {
    try {
        const { specialization, minRating, maxRate, availability, page, limit, } = req.query;
        const filters = {};
        if (specialization) {
        ...
        if (maxRate)
            filters.maxHourlyRate = Number(maxRate);
        if (availability)
 ...
 */
router.post('/find-matching', async (req, res) => {
    try {
        ...
        const teachers = await teachersService.findMatchingTeachers(studentId, limit || 10);
        res.json({
            success: true,
            data: teachers,
        });
...
router.post('/book-lesson', async (req, res) => {
    try {
        ...
        const lesson = await teachersService.bookLesson(studentId, teacherId, new Date(scheduledAt), duration, topic);
        res.json({
            success: true,
            data: lesson,
        });
 ...
 * Cancel a lesson
 */
router.post('/cancel-lesson', async (req, res) => {
    try {
        ...
        const lesson = await teachersService.cancelLesson(lessonId, userId, reason || 'User cancelled');
        res.json({
            success: true,
            data: lesson,
        });
...
router.post('/review', async (req, res) => {
    try {
        ...
        const review = await teachersService.submitReview(lessonId, studentId, rating, comment);
        res.json({
            success: true,
            data: review,
        });
 ...
 */
router.get('/analytics/:teacherId', async (req, res) => {
    try {

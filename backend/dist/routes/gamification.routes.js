...
const gamificationService = (0, gamification_service_1.getGamificationService)();
/**
...
router.post('/xp', async (req, res) => {
    try {
        ...
        const result = await gamificationService.addXP(userId, amount, source || 'activity');
        res.json({
            success: true,
            data: result,
        });
...
router.get('/leaderboard', async (req, res) => {
    try {
        ...
        const leaderboard = await gamificationService.getLeaderboard(type || 'global', period || 'weekly', userId || '', Number(limit) || 100);
        res.json({
            success: true,
            data: leaderboard,
        });
